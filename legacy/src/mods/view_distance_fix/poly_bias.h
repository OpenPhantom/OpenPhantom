 ...
 * the gradient is steep, so it only shows when the band's end lands inside the draw cut: a band
 * that saturates beyond the cut has no steep gradient anywhere on screen.
 *
 * This switch exists to test that, and it reaches the mechanism through one number: the scale
 * has exactly one reader in the image, so writing zero over it makes every polygon's bias zero and
 * changes nothing else. What it costs is what the bias is for, depth sorting between coplanar
 * surfaces, so expect flicker between overlapping faces with it off. It is a diagnostic and a
 * question, not a shipped answer.
 */
