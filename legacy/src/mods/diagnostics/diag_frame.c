...
int diag_frame_install(int level, int hitch_percent)
{
    ...
    log_info("frame timing armed at level %d: one summary a second%s. A hitch is a frame more than "
             ...
             "percentage keeps this comparable across rates, because the same build runs "
             "at 30 frames a second and at 300; the floor beside it keeps scheduler noise out "
             "once frames are short, where a fifth of the median is "
             "well under a millisecond. Measured: without "
