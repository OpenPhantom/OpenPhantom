...
static void census_report(void)
{
    ...
    log_info("drawn view turn measured over %u rendered frames, NewMouseInput=%d: roughness on the "
             ...
             "over %u frames that ran a step and %u that did not. Read the ratio: the defect "
             "puts the whole change in turn rate on the one frame in five that ran a step, so a "
             "large ratio is the stepping and a ratio near one is its absence. The median and "
             "the mean are printed beside it because they are what an "
