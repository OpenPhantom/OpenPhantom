 ...
 * the cell can be read out of its own first instruction, which is why the address appears once
 * here rather than being written down as a constant. */
...
void input_mode_resolve(void)
{
    ...
    log_info("the engine's input mode is at %08X, so the pad stick stands down whenever the "
             ...
             "than testing a flag, which is why a stick read straight from XInput has to ask",
             (unsigned)address);
}
