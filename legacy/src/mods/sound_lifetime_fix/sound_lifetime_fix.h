...
/* Exposed for the unit test: true when this owner handle points into the given stack extent. That
 * test decides whether a pinned voice is holding a local of a frame that will return. limit is the
 * low end and base the high end, as the thread information block records them. */
