...
proven correct and already running, never touched, does the rest on its very next pass over the
list. Checked every frame (`CLEANUP_EVERY_FRAMES = 1u`); an earlier build checked every 5th frame
and was field-reported as making debris "vanish faster or slower" inconsistently: the coarse
polling interval was jitter comparable in magnitude to the 0.1s threshold itself, and checking
every frame removed it.
...
`cmake --build` and `ctest` both pass clean under `/W4 /WX`, and the split was played and confirmed
post-extraction: `engine_fixes.log` shows both sites resolving (`activation_race_actor_destroy ->
00437850`, `projectile_list_tick -> 004524B9`, list head read back as `00872FB8`, matching the
address the pre-extraction hardcoded version used), both fixes arming, `view_distance_fix.dll`'s own
`actor_destroy` detour reporting the site "already detoured; found by its tail" and chaining onto
this DLL's hook regardless of load order, and both mechanisms firing during play against the known
placements by name and position (`activation race fix: refused a reason-0 destroy of "enemy076"/
"enemy077"` climbing across the session, `projectile cleanup fix: forced removal of a` firing 55
times in one session). The user confirmed the stall stays fixed.
