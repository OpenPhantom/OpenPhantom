 ...
 *
 * So the state is shared rather than hidden. The fog regime is one machine, not two. The split is
 * between a pass that runs ONCE and resolves every site, and the frame by frame work that runs
 * for the rest of the session. That is a real boundary in time and in purpose even though both
 * halves write the same record.
 *
...
typedef struct fog_regime_state {
     ...
     * `authored`; nothing here ever reads the live field back as an input, so a repeated apply
     * cannot square its own effect. */
    void               *level;
