/* cheats_fall_consequences.c: the five things the engine does to whatever comes down again, and
 * the one question that decides whether any of them happens.
 *
 * Owns fall damage, the airborne death timer, the fall distance death, the camera type latch and
 * the frozen camera target: five sites inside one retail ground-contact function, each suppressed
 * for exactly as long as jump boost is on or a granted grace is running, and only when the fall
 * has somewhere to land. It also owns the fall state itself, which is the grace deadline, the
 * floor answer and the reset a level change needs.
 *
 * SEAM. Split out of cheats_jump_boost.c, which was over the hard limit. The cut is the launch
 * against the landing: making a jump higher is one responsibility and suppressing what the engine
 * does about the fall afterwards is another. The two share no state beyond the cheat's own on
 * flag, which lives in the record every cheat file already writes, so nothing was duplicated and
 * no accessor had to be invented for the split. Nothing was renamed and no site, offset, opcode
 * or prologue size changed; the five sites moved together with the evidence for each.
 *
 * SIZE NOTE. Over 600 lines on purpose, for the reason the file this came out of already gave for
 ...
 * +0xB4, the same PLAYER_VERTICAL_VELOCITY cheats_jump_boost.c scales for jump boost itself, and
 * an accumulated fall-distance field at +0x360) and a mode check that exempts Stand and Swim:
 *
 ...
 * this point was apparently never a case retail's own camera code needed to recover from. That
 * is exactly the case jump boost now creates on purpose.
 *
 ...
 * (confirmed by get_xrefs_to on 0x0044F891 finds exactly this one caller in the whole binary), and
 ...
 * their own.
 *
 * The three sites are independent of EACH OTHER too, fall damage most of all, since a player
 * might have Unlimited health off (so hook_damage() can't be called safely, fall damage stays
 * live) while still getting fall-death immunity, which needs nothing from hook_damage() at all.
 * Each of the three is attempted and logged on its own merits; none is required for the others
 * to proceed. */
void install_fall_punishment_immunity(void)
{
     ...
     * damage and fall-death sites above, so a player could have either of those fail to resolve and
