 ...
 *
 * i.e. anchor = (anchor + at) * 0.5, an ARITHMETIC MEAN evaluated once per rendered frame.
 ...
 * plain vector add at 0x433AC1. All 63 together match exactly once, in all six shipped images,
 * including obi.exe, where the lag-constant site below finds nothing at all.
 *
 ...
 *
 * turn == 0 is what makes bapview_followYaw take its RIGID SNAP arm instead of easing. The
 * threshold is compared against a per-FRAME angle, so it scales with the frame rate: a 15 deg/s
 * turn is 0.5 deg/frame at 30 fps (eases) and 0.03 deg/frame at 500 fps (SNAPS). Around that
 * boundary the camera flips between hard-locked and eased many times a second, which is exactly
 * what "rubber-banding" looks like. Scaled by dt*30 it means the same angular RATE at any rate. */
...
/* The two cells that replace [0x4A8170] IN bapview_followYaw, and why there are TWO.
 *
 ...
 *
 *   0x418F6D  fld [c] / fmul [ebp-0x20]   <- cam    gets k
 *   0x418F76  fld [c] / fmul [ebp-0x70]   <- target gets 1-k                                    */
...
static int repoint_follow_yaw(void)
{
    ...
    if (!memory_read_u32(site + (uintptr_t)OFFSET_FOLLOW_YAW_OPERAND[0], &expected)) {
        return 0;
    }
    ...
    if (patch_repoint_operand(site + (uintptr_t)OFFSET_FOLLOW_YAW_OPERAND[0], expected,
                              (uint32_t)(uintptr_t)&follow_yaw_k) == PATCH_RESULT_OK) {
        ++repointed;
    }
    if (patch_repoint_operand(site + (uintptr_t)OFFSET_FOLLOW_YAW_OPERAND[1], expected,
                              (uint32_t)(uintptr_t)&follow_yaw_one_minus_k) == PATCH_RESULT_OK) {
        ++repointed;
    }
    return repointed;
}
...
static bool install_anchor_blend(void)
{
     ...
     * RATHER THAN CRASHING, so the bytes are re-read immediately before the write even though the
...
bool camera_compensation_install(bool compensate_anchor)
{
     ...
     * are absolute-address writes that only the retail builds carry, in obi.exe that pattern
     * finds nothing, while the anchor run is present and unique in all six shipped images. Behind
     * the early return below the anchor would silently never be patched there. */
    ...
    log_info("camera compensated - 5 immediates at %08X/%08X, %d/2 followYaw operands repointed",
             (unsigned)lag, (unsigned)pitch, repointed);
    return true;
}
