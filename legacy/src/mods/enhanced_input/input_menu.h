...
/* Two check boxes on the game's own controls screen, wired to the [enhanced_input] Strafe and
 * FreeLook keys. Both settings are live: their machinery is installed whichever way the keys
 * read, so a box only flips a gate, and neither box patches a byte when it is ticked.
 *
