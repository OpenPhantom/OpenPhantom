...
bool face_bounds_vertex_count_refused(uint32_t count, uint32_t max_vertices)
{
     ...
     * outcodes and the authored limit is 30, leaving two vertices of room between the contract
     * every caller was written against and the byte holding the saved return address. */
...
bool face_bounds_pool_would_overflow(uint32_t used, uint32_t capacity, uint32_t count)
{
     ...
     * test also keeps the subtraction below safe: after it, used is at most capacity, so
