 ...
 * that does not bounce/persist on impact (flag bit 0x2 clear). An entry that DOES persist on
 * impact, sparks, debris, anything that scatters rather than vanishing on the spot, takes neither
 * path: on impact it settles (velocity zeroed) and then simply continues to exist, still running
 * its own full per-frame world-collision trace, for as long as whatever external code is supposed
 * to set that removal flag keeps failing to do so.
 *
 ...
 * correct and already running, never touched, does the rest on its very next pass over the list.
