 ...
 * MatchDisplayRefresh and TargetFps in the [framerate_fix] section. That DLL owns the cap and
 * re-reads both about once a second, so a change here takes effect within that second, live,
 * without restarting. Nothing here calls into it, because feature DLLs in this tree do not depend
 * on each other.
 */
...
/* The match toggle, the typed limit, and the three lines of the note under them. */
#define OVERLAY_FRAMERATE_ROW_COUNT 5u
...
/* The match toggle. Answers false when the settings file could not be written; the panel shows
 * that as the row failing rather than silently doing nothing. */
